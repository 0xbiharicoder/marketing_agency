...
/**
 * Fetches the price of a token using multiple data sources
 */
export async function fetchTokenPrice(tokenInfo: TokenInfo | null): Promise<number | null> {
  ...
  try {
    ...
    // Try Birdeye API first (most reliable for Solana)
    if (BIRDEYE_API_KEY) {
      try {
        ...
        console.log(`[TokenService] Birdeye API error for ${tokenInfo.symbol}:`, birdeyeError);
      }
    }
    
    // Try CoinGecko API if available
    try {
      // For popular tokens, try CoinGecko
      const cgResponse = await fetch(`https://api.coingecko.com/api/v3/simple/token_price/solana?contract_addresses=${tokenInfo.address}&vs_currencies=usd`);
      if (cgResponse.ok) {
        const cgData = await cgResponse.json();
        const price = cgData[tokenInfo.address.toLowerCase()]?.usd;
        if (price) {
          console.log(`[TokenService] CoinGecko returned price for ${tokenInfo.symbol}: ${price}`);
          return price;
        }
      }
    } catch (cgError) {
      console.log(`[TokenService] CoinGecko error for ${tokenInfo.symbol}:`, cgError);
    }
    ...
    try {
      ...
      console.log(`[TokenService] Jupiter error for ${tokenInfo.symbol}:`, jupError);
    }
